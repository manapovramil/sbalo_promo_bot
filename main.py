...
Функциональность:
- Приветствие и "О бренде"
- Inline: «🎁 Проверить подписку и получить промокод»
- Промокоды: 4 символа A–Z/0–9, минимум 1 буква; гашение сотрудниками
- Админ = тоже сотрудник; может добавлять сотрудников
- Отзывы: рейтинг + текст + до 5 фото (в лист Feedback Google Sheets)
"""
...
SUBSCRIPTION_MIN_DAYS = int(os.getenv("SUBSCRIPTION_MIN_DAYS", "0"))  # дни с момента подписки до выдачи кода
SERVICE_ACCOUNT_JSON = os.getenv("SERVICE_ACCOUNT_JSON", "").strip()
DISCOUNT_LABEL = os.getenv("DISCOUNT_LABEL", "7%")  # отображаемая скидка
...
if not headers:
    ...
    # Досоздадим отсутствующие колонки (на будущее)
    for h in HEADERS:
        if h not in headers:
...
# Внутренние состояния и черновики
...
def is_staff(uid: int) -> bool:
    # админ считается сотрудником
    return uid in STAFF_IDS or is_admin(uid)

def add_staff_id(new_id: int) -> None:
    STAFF_IDS.add(new_id)
    # Сохраним в окружении для текущего процесса (на Render лучше задать в Settings отдельно)
...
def make_main_keyboard(user_id: int):
    ...
    if is_staff(user_id):  # сотрудники (включая админа) видят кнопку погашения
...
def get_row_by_user(user_id: int) -> Tuple[int, dict]:
    """Найти строку по UserID. Возвращает (row_index or None, record or None)."""
...
def generate_short_code() -> str:
    """4 символа A–Z/0–9, минимум 1 буква."""
...
def ensure_subscribed_since(user_id: int) -> datetime:
    """Заполнить/вернуть дату начала подписки пользователя для логики ожидания."""
    i, rec = get_row_by_user(user_id)
...
def can_issue(user_id: int) -> bool:
    """Проверка условия по времени подписки (если включено)."""
...
def issue_code(user_id: int, username: str, source: str = "subscribe") -> Tuple[str, bool]:
    """Выдать промокод (или вернуть существующий). Возврат: (code, is_new)."""
...
def redeem_code(code: str, staff_username: str) -> Tuple[bool, str]:
    """Погасить код сотрудником. True/False + сообщение пользователю."""
    for i, rec in enumerate(sheet.get_all_records(), start=2):
...
def do_check_subscription(chat_id: int, user):
    """Inline-процедура: проверка подписки и выдача промокода при необходимости."""
...
def handle_text_general(message):
    ...
    # Ожидание фото: пришёл произвольный текст
...
def handle_admin_add_staff(message):
    ...
    bot.reply_to(
        message,
        "Пришлите ID пользователя-сотрудника (цифрами), "
...
# Сформируем BASE_URL (приоритет: BASE_URL -> RENDER_EXTERNAL_HOSTNAME)
...
if __name__ == "__main__":
    # Если WEBHOOK_URL валиден — работаем через webhook; иначе — polling
    if WEBHOOK_URL:
        run_with_webhook()
    else:
